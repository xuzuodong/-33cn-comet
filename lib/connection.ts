...
export default class Connection implements IConnection {
        ...
        this.webSocket.onmessage = (event) => {
            decodeMessage(event.data).then((response) => {
                ...
                // 接收到对方用户发来的消息，回复“收到”
                    ...
                    console.log('Comet: new message received: ' + this.url)
    ...
    disconnect(): void {
        this.seq++
        if (this.debug) console.log(`Comet: disconnected actively, seq: ${this.seq}`)
    ...
    sendMessage(msg: { body: Uint8Array; uuid: string }): Promise<void> {
        return new Promise((resolve, reject) => {
            if (this.debug) console.log(`Comet: send message, seq: ${this.seq}`)
    ...
    private sendHeartBeat(): void {
        this.seq++
        if (this.debug) console.log(`Comet: heart beat, seq: ${this.seq}`)
